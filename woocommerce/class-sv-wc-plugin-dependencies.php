...
if ( ! class_exists( '\\SkyVerge\\WooCommerce\\PluginFramework\\v5_1_4\\SV_WC_Plugin_Dependencies' ) ) :
	...
	protected function add_deprecated_notices() {
		...
		if ( isset( $_GET['page'] ) && 'wc-settings' === $_GET['page'] && SV_WC_Plugin_Compatibility::is_wc_version_lt( '3.0' ) ) {
			...
			$message = sprintf(
				...
				esc_html( $this->get_wc_version() ),
				'<strong>', '</strong>',
				esc_html( $this->get_plugin()->get_plugin_name() ),
				'<a href="' . esc_url( admin_url( 'update-core.php' ) ) . '">', '</a>'
			);

			$this->get_admin_notice_handler()->add_admin_notice( 'sv-wc-deprecated-wc-version', $message, 'warning' );
		}
	}
