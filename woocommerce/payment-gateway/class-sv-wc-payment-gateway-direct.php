	...
	public function create_payment_token( $order, $response = null, $environment_id = null ) {
		...
		if ( $response->transaction_approved() ) {
			...
			if ( $this->is_credit_card_gateway() ) {
				...
				$message = sprintf( esc_html__( '%1$s Payment Method Saved: %2$s ending in %3$s (expires %4$s)', 'woocommerce-plugin-framework' ),
					$this->get_method_title(),
					$token->get_type_full(),
					$token->get_last_four(),
					$token->get_exp_date()
				...
				$message = sprintf( esc_html__( '%1$s eCheck Payment Method Saved: %2$s account ending in %3$s', 'woocommerce-plugin-framework' ),
					$this->get_method_title(),
