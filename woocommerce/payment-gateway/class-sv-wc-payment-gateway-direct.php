...
abstract class SV_WC_Payment_Gateway_Direct extends SV_WC_Payment_Gateway {
	 ...
	 * $order->customer_id             - optional payment gateway customer id (used for tokenized payments for certain gateways, etc)
	 ...
	 * @param array $args optional arguments array, can include 'environment_id' and 'customer_id'
	...
	public function get_payment_tokens( $user_id, $args = array() ) {
		...
		if ( ! isset( $args['customer_id'] ) ) {
			$args['customer_id'] = $this->get_customer_id( $user_id, $args );
		}
		...
		// gateways that don't support getting them over the API, as well as the
		// default token for gateways that do
		if ( $user_id ) {
