...
if ( ! class_exists( '\\SkyVerge\\WooCommerce\\PluginFramework\\v6_2_3\\SV_WC_Payment_Gateway_Payment_Form' ) ) :
	...
	protected function get_js_handler_args() {
		...
		if ( $this->get_gateway()->supports_card_types() ) {
			...
			if ( is_array( $card_types ) && ! empty( $card_types ) ) {
				...
				$args['enabled_card_types'] = array_map( [ 'SkyVerge\WooCommerce\PluginFramework\v6_2_3\SV_WC_Payment_Gateway_Helper', 'normalize_card_type' ], $card_types );
			}
		}
