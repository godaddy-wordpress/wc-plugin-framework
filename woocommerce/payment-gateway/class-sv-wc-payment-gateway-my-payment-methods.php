...
if ( ! class_exists( '\\SkyVerge\\WooCommerce\\PluginFramework\\v5_15_13\\SV_WC_Payment_Gateway_My_Payment_Methods' ) ) :
...
class SV_WC_Payment_Gateway_My_Payment_Methods extends Handlers\Script_Handler {
	...
	public function maybe_enqueue_styles_scripts() {
		...
		wp_enqueue_style( "$handle-v5_15_13", $this->get_plugin()->get_payment_gateway_framework_assets_url() . '/css/frontend/' . $handle . '.min.css', [ 'dashicons' ], $fw_version );

		wp_enqueue_script( "$handle-v5_15_13", $this->get_plugin()->get_payment_gateway_framework_assets_url() . '/dist/frontend/' . $handle . '.js', [ 'jquery-tiptip', 'jquery' ], $fw_version );
	}
